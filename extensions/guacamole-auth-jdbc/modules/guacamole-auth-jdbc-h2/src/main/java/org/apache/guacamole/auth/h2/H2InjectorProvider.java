 ...
 * connecting to a H2 database based on H2-specific options
 * provided via guacamole.properties.
 */
public class H2InjectorProvider extends JDBCInjectorProvider {

    @Override
    protected Injector create() throws GuacamoleException {

        // Get local environment
        H2Environment environment = new H2Environment();

        // Set up Guice injector
        return Guice.createInjector(
            new JDBCAuthenticationProviderModule(environment),
            new H2AuthenticationProviderModule(environment)
        );
