...
/**
 * A caching provider of singleton Guice instances, where the first call to
 * get() will return a new instance of the provider, while subsequent calls
 * will return that same instance.  This is a top-level class that leaves the
 * details of how the implementation is done to the hypervisor-specific
 ...
 * classloader. Within the context of the VDI extension, as long as each built
 ...
 */
public abstract class VDIInjectorProvider {
     ...
     */
    private static final AtomicReference<Injector> INJECTOR =
            new AtomicReference<Injector>(null);

    /**
     * Creates a new instance of the Guice Injector which should be used
     * across the entire VDI authentication extension. This function will
     ...
     *
     * @return
     *     A new Guice injector for use in the VDI authentication extension.
     * 
     ...
     * the injections required by the VDI authentication extension. The result
